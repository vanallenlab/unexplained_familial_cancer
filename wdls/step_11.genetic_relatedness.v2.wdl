...
task T6_calculate_pcs {
  input {
    File vcf
  }
  Int default_disk_gb = ceil(size(vcf,"GB")) * 4 + 20
  command <<<
  python3 <<CODE
  import hail as hl
  import pandas as pd
  ...
  hl.import_vcf("~{vcf}", force_bgz=True, reference_genome='GRCh38', array_elements_required=False).write("ufc.mt", overwrite=True)
  mt = hl.read_matrix_table("ufc.mt")
...
task T6_calculate_grm{
  ...
  pc_table = hl.import_table('~{pcs}', impute=True, key='#sample')
