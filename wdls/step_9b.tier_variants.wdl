...
task T2_Filter_Tier2 {
  ...
  # those that have ANY row where (missense_variant AND protein_coding)
  ids_to_remove = (
      df.groupby("ID")
        .filter(lambda g: ((g["CONSEQUENCE"].str.contains("missense_variant")) &
                           (g["BIOTYPE"] == "protein_coding")).any())
        ["ID"].unique()
  )

  # Drop those IDs from df
