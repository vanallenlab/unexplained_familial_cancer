...
task T1_analyze_inheritance {
  ...
  mask_smoking = (
      ...
      merged['smoking_history'] == 1
  )
  merged.loc[mask_smoking, 'group'] = f"Smoking {ufc_cancer.capitalize()}"

  # Familial: original_dx contains breast, family_dx contains breast
  mask_nonsmoking = (
      merged['original_dx'].fillna("").str.lower().str.contains(ufc_cancer) &
      merged['smoking_history'] == 0
  )
  merged.loc[mask_nonsmoking, 'group'] = f"Non-smoking {ufc_cancer.capitalize()}"
  ...
  # --- Print to stdout ---
  subset.to_csv("~{ufc_cancer_type}.~{PGS_ID}.analysis_4c_smoking.tsv",sep='\t',index=False)
  CODE
  >>>
