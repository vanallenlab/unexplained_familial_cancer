...
workflow SAIGE_GENE {
  ...
  call RecalibrateSaigeStats {
    input:
      ...
      output_prefix = "~{cancer_type}_prs"
  }
...
task get_covariates {
  ...
  prs_df['original_id'] = prs_df['sample'].astype(str)

  # Merge the DataFrames
  merged_df = pd.merge(sample_data, phenotypes, left_on="original_id", right_on="Sample")
  merged_df = pd.merge(merged_df, prs_df, on="original_id",how='left')
  ...
  # Set up regression: regress PGS ~ PC1 + PC2 + PC3 + PC4
  X = filtered_df[covariates]
  X = sm.add_constant(X)
  y = filtered_df['PGS']

  model = sm.OLS(y, X).fit()
  filtered_df['PGS'] = model.resid

  filtered_df['PGS'] = zscore(filtered_df['PGS'].astype(float))
  # Write to file
