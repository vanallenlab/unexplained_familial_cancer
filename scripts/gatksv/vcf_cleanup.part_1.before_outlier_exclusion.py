...
def update_sl_stats(record):
    """
    Annotates each record with SL_MEAN and SL_MAX, if missing
    """

    # If already annotated, don't do anything more
    if len(set(record.info.keys()).intersection(set('SL_MEAN SL_MAX'.split()))) == 2:
        return record

    # Otherwise, collect SLs
    sls = np.array([sdat.get('SL') for sdat in record.samples.values()])

    # Annotate missing values as necessary
    if 'SL_MEAN' not in record.info.keys():
        try:
            record.info['SL_MEAN'] = np.nanmean(sls)
        except:
            pass
    if 'SL_MAX' not in record.info.keys():
        try:
            record.info['SL_MAX'] = np.nanmax(sls)
        except:
            pass
