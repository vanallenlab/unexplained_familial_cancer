...
# -----------------------
file = "/Users/noah/Desktop/ufc_repository/results/analysis_4f_prs_utility/4f.colorectal.tsv"
...
familial_vals = pgs.loc[pgs["group"] == "Familial Colorectal", "PGS"].values

# If isolated exists, include it safely
if "Isolated Colorectal" in pgs["group"].unique():
    isolated_vals = pgs.loc[pgs["group"] == "Isolated Colorectal", "PGS"].values
else:
...
if len(isolated_vals) > 0:
    ax.plot(thresholds, isolated_curve, color="#17BECF", lw=1, alpha=0.9)
...
ax.set_xlabel("Colorectal Cancer PRS Z-Score", fontsize=7)
...
plt.savefig(
    "/Users/noah/Desktop/ufc_repository/results/analysis_4f_prs_utility/CRC.pdf",
    dpi=600
)
