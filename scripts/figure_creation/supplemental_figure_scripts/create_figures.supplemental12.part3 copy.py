...
df = df[df['Gene'].isin(["BRCA1","BRCA2"])]
...
ax.scatter(df["SD_for_gene"], df["familial_pct"],
           label="Familial", s=12, alpha=0.9, color="#b24e44")
ax.scatter(df["SD_for_gene"], df["isolated_pct"],
           label="Isolated", s=12, alpha=0.7, color="#FF6F61")
ax.scatter(df["SD_for_gene"], df["control_pct"],
           label="Control", s=12, alpha=0.8, color="#B0B0B0")
...
secax.set_xticklabels([f"{x:.2f}" for x in gene_or], fontsize=7)
