...
def main():
    ...
    roh_df = pd.read_csv(args.roh_file,sep='\t',index_col=False,header=None,names=['Chromosome','Start','End','Sample','Length'])
    phenotype_df = pd.read_csv(args.phenotype_data,sep="\t", dtype=str,index_col=False)
    ...
    samples = set(samples)
    sample_meta_df = pd.read_csv(args.sample_metadata,index_col=False,sep='\t')
    sample_meta_df['Sample'] = sample_meta_df['original_id'].astype(str)
    phenotype_df = pd.merge(phenotype_df,sample_meta_df,on='Sample',how='left')
